...
def render():
    ...
    if not st.session_state.u_selected:
        st.markdown("### 템플릿 선택")
        st.caption(f"저장된 템플릿 {len(all_tpl)}개 | 사용할 템플릿을 선택하세요")
                ...
                with cols[col_idx]:
                    ...
                    if b64:
                        ...
                        st.markdown(
                            ...
                            'color:rgba(255,255,255,0.3);font-size:28px">🖼️</div>',
                            unsafe_allow_html=True,
                        )
    ...
    <div class="info-card">
        ...
        <span style="color:#A0A0A0;font-size:12px">🖼️ 이미지존 {iz}개 &nbsp;·&nbsp; ✏️ 텍스트존 {tz}개 &nbsp;·&nbsp; {tpl['canvas_size'][0]}×{tpl['canvas_size'][1]}px</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_form:
        ...
        for i, zone in enumerate(zones):
            ...
            with st.expander(f"{icon} {label}  ({zone['w']}×{zone['h']}px)", expanded=True):
                ...
                if ztype == "image":
                    up = st.file_uploader(
                        f"이미지 선택 (권장: {zone['w']}×{zone['h']}px)",
                        type=["jpg","jpeg","png"], key=f"u_img_{i}",
                    )
                    if up:
                        raw = up.read()
                        inputs[i] = {"value": raw}
                        thumb = Image.open(io.BytesIO(raw)); thumb.thumbnail((200,200))
    ...
    with col_prev:
        ...
        if st.session_state.u_preview:
            pv_bytes = st.session_state.u_preview
            pv_img = Image.open(io.BytesIO(pv_bytes))
            st.caption(f"전체 크기: {pv_img.size[0]}×{pv_img.size[1]}px")
            show_long_image(pv_bytes, max_width=420)
        elif source_bytes:
            src_img = Image.open(io.BytesIO(source_bytes))
            st.caption(f"템플릿 원본: {src_img.size[0]}×{src_img.size[1]}px")
            show_long_image(source_bytes, max_width=420)
