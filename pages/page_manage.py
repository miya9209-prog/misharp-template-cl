...
def render():
    ...
    total      = len(all_tpl)
    total_z    = sum(len(m["zones"]) for m in all_tpl.values())
    ...
    for tid, meta in all_tpl.items():
        with st.container():
            ...
            with c1:
                ...
                if b64:
                    st.markdown(
                        f'<img src="data:image/jpeg;base64,{b64}" style="width:100%;border-radius:6px">',
            ...
            with c2:
                ...
                tags = " ".join([
                    f'<span style="background:rgba({"200,168,118" if z["type"]=="image" else "100,160,230"},0.15);'
                    f'color:{"#C8A876" if z["type"]=="image" else "#78a8f0"};'
                    f'padding:3px 10px;border-radius:4px;font-size:11px;margin-right:4px;font-weight:600">'
